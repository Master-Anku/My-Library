...
async function getCurrentWeather(cityName) {
    ...
    const apiKey = '83ac9345e5cc138db5e819c64015bd0f'; // သင့် API Key ကို ထည့်ပါ
...
function displayWeather(cityName, currentWeather, futureWeather) {
    ...
    document.getElementById('temperature').textContent = `အပူချိန်: ${currentWeather.main.temp}°C`;
    document.getElementById('humidity').textContent = `စိုထိုင်းဆ: ${currentWeather.main.humidity}%`;
    document.getElementById('windSpeed').textContent = `လေတိုက်နှုန်း: ${currentWeather.wind.speed} m/s`;
    ...
    for (let i = 0; i < futureWeather.list.length; i += 8) { // Skip to next day's forecast
        ...
        forecastDiv.innerHTML = `
            <p>ရက်စွဲ: ${new Date(day.dt_txt).toLocaleDateString()}</p>
            <img src="http://openweathermap.org/img/wn/${day.weather[0].icon}.png" alt="Weather Icon">
            <p>အပူချိန်: ${day.main.temp}°C</p>
            <p>လေတိုက်နှုန်း: ${day.wind.speed} m/s</p>
            <p>စိုထိုင်းဆ: ${day.main.humidity}%</p>
        `;
